...
const form_activation = () => {
  ...
  const formEvent = (e) => {
    ...
    if (
      ...
      let aux = new Project(
        project_form.elements["project-title"].value,
      ...
      document
        .getElementsByClassName(`project-form`)[0]
...
export function ProjectEdit(project) {
  ...
  const formEventEdit = (e) => {
    ...
    if (
      ...
      localStorage.setItem(project.id, JSON.stringify(aux));
      document
        .getElementsByClassName(`project-form`)[0]
        .classList.add(`hidden`);
...
export function projectEditView(project) {
  ...
  if (projectObject.low === true) document.getElementById(`low`).checked = true;

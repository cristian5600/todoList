...
export const showProjects = () => {
  ...
  BTN.addEventListener(`click`, () => {
    ...
    if (localStorage.getItem(`projects-index`) != null)
      for (
        let i = 1;
        i <= parseInt(localStorage.getItem(`projects-index`));
        i++
      ) {
...
function addProjectToElement(project, element, id) {
  ...
  if (project.low === true) importance.innerHTML = `Importance:low`;
  else if ((project.medium = true))
    importance.innerHTML = `Importance:moderate`;
  else if ((project.high = true)) importance.innerHTML = `Importance:high`;
  ...
  if (btn.classList.contains(`remove`)) {
    btn.addEventListener(`click`, () => {
      ...
      for (
        ...
        if (localStorage.getItem(`p-${i}`) === null) condition = true;
